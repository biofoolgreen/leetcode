...
Date: 2021-06-01 20:41:48
LastEditTime: 2021-06-01 23:06:55
...
class Solution:
    def canEat(self, candiesCount: List[int], queries: List[List[int]]) -> List[bool]:
        ans = []
        num_candies_type = len(candiesCount)
        max_days = [0]*num_candies_type
        max_days[0] = candiesCount[0]
        for i in range(1, num_candies_type):
            max_days[i] = max_days[i-1]+candiesCount[i]
        print(max_days)
        for tp, day, cap in queries:
            if tp == 0:
                min_days = 1
            else:
                min_days = max_days[tp-1] // cap + 1
            print(min_days, max_days[tp])
            if min_days<=day<=max_days[tp]:
                ans.append(True)
            else:
                ans.append(False)
        return ans
